...
summary(lm(formula = TaxAbundance ~ Depth_m, data = qiime2_taxon))
summary(lm(formula = TaxAbundance ~ TotAbundance, data = mothur_taxon))
summary(lm(formula = TaxAbundance ~ TotAbundance, data = qiime2_taxon))
...
ggplot(mothur_taxon, aes(x=Depth_m, y=TaxAbundance)) + 
  ...
  geom_smooth(method=lm)+
  scale_colour_distiller(palette = "RdYlGn") + # replace if using alt colours
  labs(x = "Depth (m)", y = "Tax Abundance", 
       colour = expression(paste("[O"[2], "] (", mu, "M)"))) 
...
summary(lm(formula = Otu0308 ~ Depth_m, data = mothur_otus_total))
summary(lm(formula = Otu0666 ~ O2_uM, data = mothur_otus_total))
summary(lm(formula = Otu0666 ~ Depth_m, data = mothur_otus_total))
